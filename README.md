...
## The Product
To begin this project, I first needed to lay out the steps and micro projects within, to be able to make sure this runs correctly. The first step involved using Twitter’s API. Like in Project 2a, I needed to leverage the power of pulling user tweets to be able to get the power rankings for that week, along with the schedule of games. `getTweets.py` is attached above and pulls the attachment from the tweet from the NFL for that specific week. Whenever I run `getTweets.py`, I am prompted to authenticate my request in the terminal by copying and pasting a given web address into my browser. I am then given a pin, which gets inputted back into the terminal and my request can be finished. This now gives me the power ranking information. I modified the code a bit, so instead of printing out the information in my terminal, it is written into PowerRankings.json, which I have also attached. Using the link in the json file, I can then update the created dictionary in `WeeklyFavorites.py`, to show the current power rankings. Next I need to do something similar, but now pulling this weeks schedule of games. I am now going to run `getWeek5Schedule.py` and again put that into a json file, this time named WeeklySchedule.json. I can now take this information and put into a .txt file. This .txt file can then be read by ‘WeeklyFavorites.py’ to get the matchups for that week. Now, looking into `WeeklyFavorites.py`, we can see that once we have our power rankings and matchups, we can use this information to output the favorite. This output is put into Week5Favorites.txt and can be utilized by users in my user stories. We can see the output for this week below. I believe that this type of product can be very useful for many different people and even though this is just an MVP, this product could be expanded on even more in the future. 


![Output](https://user-images.githubusercontent.com/74614080/136598692-dc83347e-6334-4a8c-b2fe-cca3419fe8ce.png)
